...
function Invoke-AidosRuntimeProjectManagerTick {
    ...
    foreach($candidate in $ordered){
        ...
        if([bool]$selection.activatable){
            ...
            elseif([string]$selection.actor_identity-eq'EXECUTION_AGENT' -and [string]$selection.action-eq'DISPATCH_EXECUTION'){try{$activation=Invoke-AidosAutonomousWorkerDispatch -Project $candidate.project -Push:$Push -WorkerInvoker $WorkerInvoker;$status=if([string]$activation.status-eq'PROFILE_ADAPTER_REQUIRED'){'ACTOR_ADAPTER_REQUIRED'}else{'WORKER_DISPATCHED'}}catch{$status='ACTIVATION_ERROR';$activation=[pscustomobject]@{error=$_.Exception.Message}}}
