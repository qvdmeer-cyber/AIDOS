...
function Get-AidosPendingIntegrationIntents {
    ...
    $dir=Get-AidosIntegrationIntentRoot $ProjectRoot;if(-not(Test-Path -LiteralPath $dir -PathType Container)){return @()}
...
function Test-AidosIntegrationPathAuthorized {
    ...
    $relative=ConvertTo-AidosNormalizedRelativePath $Path
    ...
    foreach($allowedRaw in @($Execution.authority.filesystem_write)){
        ...
        $allowed=ConvertTo-AidosNormalizedRelativePath $allowed;$allowed=$allowed.TrimEnd('/')
...
function Get-AidosIntegrationChangedPaths {
    [CmdletBinding()]param([Parameter(Mandatory)][string]$ProjectRoot)
    $root=Resolve-AidosFileSystemPath $ProjectRoot;$status=Invoke-AidosGit $root @('status','--porcelain=v1','--untracked-files=all');if($status.ExitCode-ne0){throw 'Unable to inspect Worker integration worktree.'}
    $paths=[Collections.Generic.List[string]]::new();foreach($line in @($status.Output)){if([string]::IsNullOrWhiteSpace([string]$line)){continue};$path=([string]$line).Substring(3).Trim();if($path-match' -> '){$path=($path-split' -> ')[-1]};$paths.Add((ConvertTo-AidosNormalizedRelativePath $path))};@($paths)
}
function Invoke-AidosPassIntegration {
    ...
    $intent=Read-AidosJson $intentPath;if([string]$intent.status-eq'APPLIED'){return [pscustomobject][ordered]@{status='ALREADY_APPLIED';review_id=$ReviewId;commit=[string]$intent.integration_commit}}
    $review=Read-AidosReviewRecord -ProjectRoot $root -ReviewId $ReviewId;if(-not$review.decision -or [string]$review.decision.outcome-ne'PASS'){throw 'Integration requires an accepted PASS review decision.'}
    ...
    $executionPath=Get-AidosExecutionArtifactPath -ProjectRoot $root -ExecutionId ([string]$intent.execution_id) -Revision ([int]$intent.revision);if(-not(Test-Path -LiteralPath $executionPath -PathType Leaf)){throw 'Integration Execution artifact is missing.'}
    ...
    if([string]$headMessage-eq$integrationMessage){$integrationCommit=(Invoke-AidosGit $root @('rev-parse','HEAD')).Output|Select-Object -First 1}else{
        $paths=@(Get-AidosIntegrationChangedPaths -ProjectRoot $root);if($paths.Count-eq0){throw 'PASS review has no Worker/integration delta to persist.'}
        ...
        $commit=Invoke-AidosGit $root @('commit','-m',$integrationMessage);if($commit.ExitCode-ne0){throw 'AIDOS integration commit failed.'};$integrationCommit=(Invoke-AidosGit $root @('rev-parse','HEAD')).Output|Select-Object -First 1
    }
