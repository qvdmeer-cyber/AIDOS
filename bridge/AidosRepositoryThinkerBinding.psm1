...
function Get-AidosRepositoryThinkerCurrentConversationFromRoot {
    ...
    foreach($document in $documents){
        ...
        $score=0
        if($url.IndexOf('chatgpt.com',[StringComparison]::OrdinalIgnoreCase)-ge0){$score+=1}
        if($url.IndexOf('/c/',[StringComparison]::OrdinalIgnoreCase)-ge0){$score+=10}
        $candidates.Add([pscustomobject][ordered]@{element=$document;title=$title;url=$url;score=$score})
    }
    ...
    $best=$ordered[0]
    $ties=@($ordered|Where-Object {[int]$_.score-eq[int]$best.score})
    if($ties.Count-ne1){throw 'Active ChatGPT conversation document remains ambiguous.'}
...
function Test-AidosRepositoryThinkerActionableElement {
    ...
    foreach($pattern in @(
        ...
        try{if($Element.TryGetCurrentPattern($pattern,[ref]([object]$null))){return $true}}catch{}
    }
    $false
}
...
function Invoke-AidosRepositoryThinkerActionableElement {
    [CmdletBinding()]
    param([Parameter(Mandatory)]$Element)
    try{$p=$Element.GetCurrentPattern([System.Windows.Automation.SelectionItemPattern]::Pattern);if($p){$p.Select();return 'SELECTION_ITEM'}}catch{}
    try{$p=$Element.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern);if($p){$p.Invoke();return 'INVOKE'}}catch{}
    try{$p=$Element.GetCurrentPattern([System.Windows.Automation.LegacyIAccessiblePattern]::Pattern);if($p){$p.DoDefaultAction();return 'LEGACY_DEFAULT_ACTION'}}catch{}
...
function Find-AidosRepositoryThinkerConversationAction {
    ...
    foreach($element in @($RootElement.FindAll([System.Windows.Automation.TreeScope]::Subtree,[System.Windows.Automation.Condition]::TrueCondition))){
        ...
        if(-not[string]::Equals($name,$ConversationTitle,[StringComparison]::Ordinal)){continue}
        $current=$element
        for($level=0;$level-lt5 -and $current;$level++){
            if(Test-AidosRepositoryThinkerActionableElement -Element $current){
                $key=('{0}|{1}|{2}' -f [string]$current.Current.AutomationId,[string]$current.Current.ControlType.ProgrammaticName,[string]$current.Current.Name)
                $candidates.Add([pscustomobject][ordered]@{element=$current;level=$level;key=$key})
                break
            }
    ...
    $ordered=@($candidates|Sort-Object level)
    $best=$ordered[0]
    $ties=@($ordered|Where-Object {[int]$_.level-eq[int]$best.level}|Group-Object key|ForEach-Object {$_.Group[0]})
    if($ties.Count-ne1){throw "Pinned ChatGPT conversation '$ConversationTitle' is ambiguous in the active app."}
    $best.element
}
    ...
    [pscustomobject]@{
        ...
        ActivateConversation={
            ...
            for($attempt=0;$attempt-lt50;$attempt++){
                ...
                if([string]::Equals([string]$observed.url,[string]$Binding.conversation_url,[StringComparison]::Ordinal)){
                    return [pscustomobject][ordered]@{status='ACTIVATED';method=$method;conversation=$observed;context=$Context}
                }
            }
...
function Bind-AidosRepositoryThinkerConversation {
    ...
    param(
        ...
        [Parameter(Mandatory)][string]$ExpectedConversationTitle,
        [string]$ProcessName='ChatGPT Classic',
        [object]$Backend
    )
    if(-$Backend){$Backend=New-AidosRepositoryThinkerWindowsBackend -ProcessName $ProcessName}
    $context=& $Backend.GetProcessContext $ProcessName
    $conversation=& $Backend.GetCurrentConversation $context
    if(-not[string]::Equals([string]$conversation.title,$ExpectedConversationTitle,[StringComparison]::Ordinal)){
        throw "Active ChatGPT conversation title '$($conversation.title)' does not equal required project title '$ExpectedConversationTitle'. Rename and pin the conversation before binding."
    }
    if([string]::IsNullOrWhiteSpace([string]$conversation.url)){throw 'Active ChatGPT conversation URL is unavailable.'}
    $now=[DateTimeOffset]::UtcNow.ToString('o')
    $binding=[pscustomobject][ordered]@{
        schema_version='0.1'
        binding_type='MANUAL_PROJECT_CHATGPT_CONVERSATION'
        project_id=$ProjectId
        repository=$Repository
        process_name=$ProcessName
        conversation_title=$ExpectedConversationTitle
        conversation_url=[string]$conversation.url
        status='BOUND'
        bound_at=$now
        updated_at=$now
    }
...
function Invoke-AidosRepositoryThinkerTrigger {
    ...
    $projectId=[string]$Handoff.metadata.project_id
    $handoffId=[string]$Handoff.metadata.handoff_id
    ...
    if($existing -and [string]$existing.status-eq'FAILED' -and $existing.retry_after){
        $retry=[DateTimeOffset]::Parse([string]$existing.retry_after)
    ...
    try{
        ...
        $context=& $Backend.FocusConversation $context $binding
