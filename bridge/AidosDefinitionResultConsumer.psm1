...
function Resolve-AidosDefinitionActorSourceRef {
    ...
    $allowed=($relative -eq '.aidos/documentation/PROJECT_BASELINE.json' -or $relative -eq '.aidos/documentation/PROJECT_ACCESS.json' -or $relative -eq '.aidos/evidence/EVIDENCE_INVENTORY.json' -or $relative -eq '.aidos/profile/PROJECT_APPLICABILITY.json' -or $relative -eq 'AGENTS.md' -or $relative.StartsWith('docs/',[StringComparison]::Ordinal) -or $relative.StartsWith((".aidos/definitions/{0}/v{1}/" -f $DefinitionId,$DefinitionVersion),[StringComparison]::Ordinal))
...
function Assert-AidosDefinitionSourceRefs {
...
function New-AidosDefinitionHumanInputRequest {
    ...
    if([string]$Proposal.authority_classification -notin @('HUMAN_REQUIRED','AUTO_DECIDABLE')){throw 'Definition Human Input must preserve HUMAN_REQUIRED or failed AUTO_DECIDABLE authority.'}
    ...
    $sourceRefs=Assert-AidosDefinitionSourceRefs -ProjectRoot $root -AidosRoot $AidosRoot -SourceRefs @($Proposal.source_refs) -DefinitionId $definitionId -DefinitionVersion $definitionVersion
    ...
    $request=[ordered]@{
        contract_version='0.1.0';request_id=$requestId;project_id=[string]$Project.project_id;workstream_id=$null;phase='DEFINITION';request_type=[string]$Proposal.request_type;status='WAITING';
        context_summary=[string]$Proposal.context_summary;question=[string]$Proposal.question;options=@($options|ForEach-Object {[ordered]@{option_id=[string]$_.option_id;label=[string]$_.label;description=if($null-eq$_.description){$null}else{[string]$_.description}}});
        authority_classification=[string]$Proposal.authority_classification;decision_assessment_ref=if([string]::IsNullOrWhiteSpace([string]$Proposal.decision_assessment_ref)){$null}else{[string]$Proposal.decision_assessment_ref};auto_define_stop_reason=[string]$Proposal.auto_define_stop_reason;
        ...
        requested_by=[ordered]@{actor='DEFINITION_AGENT';model=$null;session_id=$null};resume_actor_role='THINKER';response=$null;evidence_refs=@($Proposal.evidence_refs|ForEach-Object {[string]$_});source_refs=@($sourceRefs);created_at=$now;updated_at=$now
    }
    if([string]::IsNullOrWhiteSpace([string]$request.context_summary)-or[string]::IsNullOrWhiteSpace([string]$request.question)-or[string]::IsNullOrWhiteSpace([string]$request.auto_define_stop_reason)){throw 'Definition Human Input request text fields are incomplete.'}
...
function Invoke-AidosDefinitionThinkerResultConsumer {
    ...
    $bound=Read-AidosRuntimeActorAssignment -ProjectRoot $root -AssignmentId ([string]$ActorResult.assignment_id);$assignment=$bound.assignment
    ...
    $output=$ActorResult.result
    if($null-eq$output -or [string]$output.result_type-ne'DEFINITION_THINKER_OUTPUT'){throw 'Definition Thinker result_type mismatch.'}
    $definitionId=[string]$state.definition_id;$definitionVersion=[int]$state.definition_version
    $setApplicability=Join-Path ([IO.Path]::GetFullPath($AidosRoot)) 'tools/Set-AidosDefinitionApplicabilitySurface.ps1'
    $setSurface=Join-Path ([IO.Path]::GetFullPath($AidosRoot)) 'tools/Set-AidosDefinitionSurface.ps1'
    $newAutoDecision=Join-Path ([IO.Path]::GetFullPath($AidosRoot)) 'tools/New-AidosDefinitionAutoDecision.ps1'
    $testAutoDecision=Join-Path ([IO.Path]::GetFullPath($AidosRoot)) 'tools/Test-AidosAutoDecision.ps1'
    $testApplicability=Join-Path ([IO.Path]::GetFullPath($AidosRoot)) 'tools/Test-AidosDefinitionApplicability.ps1'
    ...
    $appSeen=[System.Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach($resolution in @($output.applicability_resolutions)){
        ...
        $refs=Assert-AidosDefinitionSourceRefs -ProjectRoot $root -AidosRoot $AidosRoot -SourceRefs @($resolution.source_refs) -DefinitionId $definitionId -DefinitionVersion $definitionVersion -RequireAny
    ...
    if($humanResolutions.Count-gt1){throw 'Definition Thinker may surface at most one HUMAN_REQUIRED decision per result.'}
    if($humanResolutions.Count-eq1 -and $null-eq$output.human_input_request){throw 'HUMAN_REQUIRED surface requires human_input_request.'}
    if($humanResolutions.Count-eq0 -and $null-ne$output.human_input_request){throw 'human_input_request requires a matching HUMAN_REQUIRED surface.'}
    $surfaceSeen=[System.Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    $humanRequest=$null
    foreach($resolution in @($output.surface_resolutions)){
        $surfaceId=[string]$resolution.surface_id;if(-not$surfaceSeen.Add($surfaceId)){throw "Duplicate Definition surface resolution '$surfaceId'."}
        $authority=[string]$resolution.authority_classification;$status=[string]$resolution.status
        $refs=Assert-AidosDefinitionSourceRefs -ProjectRoot $root -AidosRoot $AidosRoot -SourceRefs @($resolution.source_refs) -DefinitionId $definitionId -DefinitionVersion $definitionVersion -RequireAny:($authority -in @('SYSTEM_INVARIANT','REPO_VERIFIABLE','AUTO_DECIDABLE'))
        $decisionRef=$null
        switch($authority){
            ...
            'AUTO_DECIDABLE' {
                if($null-eq$resolution.auto_decision){throw 'AUTO_DECIDABLE surface requires auto_decision payload.'};if($status-ne'COMPLETE'){throw 'Policy-valid AUTO_DECIDABLE surface must close COMPLETE.'}
                $auto=$resolution.auto_decision
                $decision=& $newAutoDecision -ProjectRoot $root -ContractsRoot $ContractsRoot -ProjectId ([string]$Project.project_id) -DefinitionId $definitionId -DefinitionVersion $definitionVersion -TargetType DEFINITION_SURFACE -SurfaceId $surfaceId -ChosenValueJson ($auto.chosen_value|ConvertTo-Json -Depth 100 -Compress) -AlternativesJson (@($auto.alternatives_considered)|ConvertTo-Json -Depth 100 -Compress) -Rationale ([string]$auto.rationale) -AssessmentJson ($auto.assessment|ConvertTo-Json -Depth 100 -Compress) -DecidedByActor DEFINITION_AGENT
                $decisionRef=[string]$decision.decision_ref
                & $testAutoDecision -ProjectRoot $root -ContractsRoot $ContractsRoot -DecisionPath (Join-Path $root $decisionRef)|Out-Null
            }
            'HUMAN_REQUIRED' {
                if($null-ne$resolution.auto_decision){throw 'HUMAN_REQUIRED surface must not persist an Auto Decision.'};if($status-ne'DECISION_REQUIRED'){throw 'HUMAN_REQUIRED surface must remain DECISION_REQUIRED.'}
                if([string]$output.human_input_request.surface_id -ne $surfaceId){throw 'Human Input request surface binding mismatch.'}
    ...
    $appCheck=& $testApplicability -ProjectRoot $root -DefinitionId $definitionId -DefinitionVersion $definitionVersion -NoExit
    if(-not[bool]$appCheck.pass){throw "Definition Applicability validation failed after actor result: $(@($appCheck.errors)-join'; ')"}
    $progressCheck=& $testProgress -ProjectRoot $root -DefinitionId $definitionId -DefinitionVersion $definitionVersion -NoExit
    if(-not[bool]$progressCheck.pass){throw "Definition Progress validation failed after actor result: $(@($progressCheck.errors)-join'; ')"}
...
Export-ModuleMember -Function Resolve-AidosDefinitionActorSourceRef,Assert-AidosDefinitionSourceRefs,New-AidosDefinitionHumanInputRequest,Invoke-AidosDefinitionThinkerResultConsumer
