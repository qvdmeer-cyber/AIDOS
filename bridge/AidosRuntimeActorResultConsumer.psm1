...
function Invoke-AidosProjectApplicabilityResultConsumer {
    ...
    param(
        ...
        [Parameter(Mandatory)]$ActorResult,
    ...
    Assert-AidosActorSourceRefs -ProjectRoot $root -SourceRefs $sourceRefs|Out-Null
    ...
    $selectionSource=[string]$proposal.selection_source
    if([string]::IsNullOrWhiteSpace($selectionSource)){$selectionSource='BASELINE_DERIVED'}
...
function Invoke-AidosRuntimeActorResultConsumerTick {
    ...
    foreach($project in @(Get-AidosConsumerRuntimeProjects -RegistryRoot $RegistryRoot)){
        ...
        foreach($file in @(Get-ChildItem -LiteralPath $transportRoot -Filter '*.json' -File|Sort-Object Name)){
            if($processed-ge$MaxItems){break}
            $transport=Get-Content -LiteralPath $file.FullName -Raw -Encoding UTF8|ConvertFrom-Json -Depth 50
            if([string]$transport.status-ne'COMPLETED'){continue}
            try{
                if([string]::IsNullOrWhiteSpace([string]$transport.result_ref)){throw 'Completed actor transport has no result_ref.'}
                $resultPath=Join-Path ([string]$project.local_root) ([string]$transport.result_ref)
                if(-not(Test-Path -LiteralPath $resultPath -PathType Leaf)){throw 'Completed actor result file is missing.'}
                ...
                $outcome=switch([string]$assignment.assignment.action){
                    ...
                    default {throw "No Core consumer exists yet for runtime actor action '$($assignment.assignment.action)'."}
                }
                ...
                $results.Add([pscustomobject][ordered]@{project_id=[string]$project.project_id;assignment_id=[string]$transport.assignment_id;status='CONSUME_ERROR';error=$_.Exception.Message})
            }
            $processed++
        }
    }
