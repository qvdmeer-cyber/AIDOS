...
function New-AidosExecutionFromAcceptedDefinition {
    ...
    $execution=[ordered]@{
        ...
        executor_profile=[ordered]@{model='codex-cli-default';reasoning_effort='medium'}
...
function Invoke-AidosAutonomousExecutionValidation {
    ...
    $validators=@()
    foreach($validator in @($Execution.validators)){
        try{$validators+=Invoke-AidosRegisteredValidator -ProjectRoot $ProjectRoot -Validator ([string]$validator)}catch{$validators+=[pscustomobject][ordered]@{validator=[string]$validator;passed=$false;exit_code=$null;output=@($_.Exception.Message)}}
    }
...
function Invoke-AidosAutonomousCodexExecution {
    ...
    $runtime=Resolve-AidosAutonomousCodexRuntime -ProjectRoot $root
    $lease=Acquire-AidosExecutionLease -ProjectRoot $root -ExecutionId ([string]$execution.execution_id) -Revision ([int]$execution.revision)
    $directory=Split-Path -Parent $executionPathResolved
    ...
    $arguments=Get-AidosAutonomousCodexArguments -Runtime $runtime -Execution $execution -PromptText $promptText
    $command=Get-AidosCodexCommand -Runtime $runtime -ProjectRoot $root -CodexArguments $arguments
    ...
    if($result.process_succeeded-and$sessionId){
        Set-AidosState -ProjectRoot $root -NewState TERMINAL_PENDING -Actor BRIDGE -Patch $statePatch|Out-Null
        $validation=Invoke-AidosAutonomousExecutionValidation -ProjectRoot $root -Execution $execution
...
function Invoke-AidosAutonomousWorkerDispatch {
    ...
    $root=Resolve-AidosFileSystemPath ([string]$Project.local_root)
    $plan=New-AidosExecutionFromAcceptedDefinition -Project $Project -Push:$Push
    ...
    try{$persist=Invoke-AidosPreparationGitPersistence -Project $Project -CommitMessage ("AIDOS persist execution $($plan.execution.execution_id) result") -Push:$Push}catch{$persist=[pscustomobject][ordered]@{status='ERROR';error=$_.Exception.Message}}
