...
function Submit-AidosProjectGoal {
    [CmdletBinding()]
    param([Parameter(Mandatory)]$Project,[Parameter(Mandatory)][string]$Goal,[ValidateSet('CHATGPT_OPERATOR','HUMAN')][string]$SubmittedBy='CHATGPT_OPERATOR',[switch]$AllowNonIdleState,[switch]$SkipDefinitionWorkspace,[switch]$Push)
    ...
    if(-not$AllowNonIdleState){
        if([string]$state.state-ne'IDLE'){throw "A new project goal requires IDLE state; current state is '$($state.state)'."}
    }elseif([string]$state.state-ne'IDLE' -and [string]$state.state-ne'WAITING_DEFINITION'){
        throw "A gateway goal bypass only permits IDLE or WAITING_DEFINITION state; current state is '$($state.state)'."
    }
    ...
    try{
        if($AllowNonIdleState -and [string]$state.state-eq'WAITING_DEFINITION'){
            $newState=$state
        }else{
        ...
        $workspace=if($SkipDefinitionWorkspace){[pscustomobject]@{status='SKIPPED';definition_id=$definitionId;definition_version=1}}else{Ensure-AidosDefinitionWorkspace -ProjectRoot $root}
