...
function Get-AidosHumanInputProjectId {
    ...
    if(Test-Path -LiteralPath $profilePath -PathType Leaf){
        return [string](Read-AidosJson $profilePath).project_id
    }
    $baselinePath=Join-Path $root '.aidos/documentation/PROJECT_BASELINE.json'
    if(Test-Path -LiteralPath $baselinePath -PathType Leaf){
        return [string](Read-AidosJson $baselinePath).project_id
    }
...
function Submit-AidosHumanInputResponse {
    ...
    Invoke-AidosExclusive -ProjectRoot $root -ScriptBlock {
        ...
        $request.response=[pscustomobject][ordered]@{
            responded_by=$RespondedBy
            responded_at=$now
            selected_option_id=if([string]::IsNullOrWhiteSpace($SelectedOptionId)){$null}else{$SelectedOptionId}
        ...
        $resume=[ordered]@{
            schema_version='0.1'
            resume_id=[guid]::NewGuid().ToString()
            request_id=$RequestId
            project_id=$projectId
            workstream_id=$request.workstream_id
            phase=$request.phase
            resume_actor_role=$request.resume_actor_role
            binding=$request.binding
            response_ref=[IO.Path]::GetRelativePath($root,$requestPath).Replace('\','/')
            status='PENDING'
            created_at=$now
            updated_at=$now
            applied_at=$null
        ...
        [pscustomobject][ordered]@{
            status='RESOLVED'
            request=$request
            resume_ref=[IO.Path]::GetRelativePath($root,$resumePath).Replace('\','/')
            resume=[pscustomobject]$resume
        }
    }
}
