...
function New-TestRuntimeProject {
    param([string]$Base,[string]$ProjectId,[string]$State='IDLE',[string]$ControlMode='RUNNING')
...
try {
    ...
    foreach($entry in @(
        @{id='RUNTIME-IDLE';root=$idleRoot},@{id='RUNTIME-WAIT';root=$waitRoot},@{id='RUNTIME-PAUSED';root=$pausedRoot}
    )){
    ...
    Assert-RuntimeManager ($projects.Count -eq 3) 'all promoted runtime projects are discovered'

    $idle=Get-AidosRuntimeNextActor -ProjectRoot $idleRoot
    Assert-RuntimeManager ($idle.actor_identity -eq 'DEFINITION_AGENT' -and $idle.action -eq 'START_DEFINITION' -and $idle.activatable) 'IDLE selects Definition Thinker'
    ...
    Assert-RuntimeManager ($global:AidosActivatedProject -eq 'RUNTIME-IDLE') 'manager selects actionable IDLE project over non-activatable projects'
