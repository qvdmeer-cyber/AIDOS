...
Assert-Reload ($text.IndexOf("if([string]`$task.State -eq 'Disabled')",[StringComparison]::Ordinal) -ge 0 -and $text.IndexOf("status='ALREADY_DISABLED'",[StringComparison]::Ordinal) -ge 0) 'Limited watchdog reload treats an already-disabled legacy task as an idempotent read-only success'
