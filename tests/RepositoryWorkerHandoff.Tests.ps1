...
try{
    ...
    Git $repo @('config','user.email','aidos@example.invalid')|Out-Null;Git $repo @('config','user.name','AIDOS Tests')|Out-Null;Git $repo @('remote','add','origin','https://github.com/example/worker.git')|Out-Null
    ...
    Git $repo @('add','.')|Out-Null;Git $repo @('commit','-m','fixture')|Out-Null
    ...
    Assert-Worker ((Git $repo @('log','-1','--pretty=%s'))[0]-match'publish Worker handoff') 'Worker assignment is committed immediately'
