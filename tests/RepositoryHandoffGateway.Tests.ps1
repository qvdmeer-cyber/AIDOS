...
$script:passed=0
function Assert-Gateway([bool]$Condition,[string]$Message){
    if(-not$Condition){throw "ASSERTION FAILED: $Message"}
    $script:passed++
}
function Invoke-Git([string]$Repo,[string[]]$Arguments){
    $output=@(& git -C $Repo @Arguments 2>&1)
    if($LASTEXITCODE-ne0){throw "git $($Arguments -join ' ') failed: $($output -join '; ')"}
    @($output)
}
...
$stateRoot=Join-Path $temp 'gateway'
...
try{
    ...
    Invoke-Git $projectRoot @('remote','add','origin','https://github.com/example/project.git')|Out-Null
    ...
    $assignment=[pscustomobject][ordered]@{
        ...
        binding=[pscustomobject][ordered]@{project_state='WAITING_DEFINITION';definition_id='DEF-1';definition_version=1;execution_id=$null;revision=$null;review_id=$null}
    ...
    Assert-Gateway ([int]$source.status_code-eq200 -and [string]$source.body.content-eq'# Product') 'authorized project source can be fetched'
    ...
    $retry=Invoke-AidosRepositoryHandoffGatewayRequest -Method POST -Path '/v1/projects/PROJECT-1/results' -Body $request -PresentedKey $loaded.api_key -ExpectedKey $loaded.api_key -RegistryRoot $registryRoot -AidosRoot $root
