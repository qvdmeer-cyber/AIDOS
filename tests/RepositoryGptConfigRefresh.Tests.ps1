...
$hostState=Join-Path $temp 'host';$bridge=Join-Path $temp 'bridge'
New-Item -ItemType Directory -Path $hostState,$bridge -Force|Out-Null
try{
    $openapi=Join-Path $hostState 'OPENAPI.json';$instructions=Join-Path $hostState 'GPT_INSTRUCTIONS.md'
    [ordered]@{schema_version='0.2';public_url='https://aidos.tail1234.ts.net';openapi_path=$openapi;instructions_path=$instructions;bridge_state_root=$bridge;process_name='ChatGPT Classic Test'}|ConvertTo-Json -Depth 20|Set-Content -LiteralPath (Join-Path $hostState 'CONFIG.json') -Encoding utf8NoBOM
    ...
    [ordered]@{schema_version='0.1';status='RUNNING'}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $hostState 'STATUS.json') -Encoding utf8NoBOM
    Assert-RefreshThrows {& $refresh -StateRoot $hostState|Out-Null} 'Stop the AIDOS Repository Handoff Host' 'refresh refuses to race a running host'

    [ordered]@{schema_version='0.1';status='STOPPED'}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $hostState 'STATUS.json') -Encoding utf8NoBOM
    $result=(& $refresh -StateRoot $hostState)|ConvertFrom-Json -Depth 50
